...
def add_skeleton_layer(
    skeleton: Union[navis.TreeNeuron, navis.NeuronList, pd.DataFrame],
    scene: Dict[str, Any]
    ) -> Dict[str, Any]:
    """
    Add a neuron skeleton as a new annotation layer to a scene.

    Parameters
    ----------
    skeleton : navis.TreeNeuron | navis.NeuronList | pd.DataFrame
        The neuron skeleton to add. Must be a single TreeNeuron, a NeuronList of length 1, or a DataFrame of nodes.
        Coordinates are expected in nanometers and will be converted to voxels (FlyWire convention: [4, 4, 40] nm).
    scene : dict
        Scene dictionary to which the annotation layer will be added. This dict is copied and not modified in-place.

    Returns
    -------
    dict
        Modified scene dictionary with the new annotation layer added.

    Raises
    ------
    TypeError
        If scene is not a dict, or skeleton is not a supported type.
    ValueError
        If a NeuronList with more than one neuron is provided.
    """
    if not isinstance(scene, dict):
        raise TypeError(f'`scene` must be dict, got "{type(scene)}"')
    scene = scene.copy()

    # Accept only TreeNeuron, NeuronList (len 1), or DataFrame
    ...
    if not isinstance(skeleton, (navis.TreeNeuron, pd.DataFrame)):
        raise TypeError(f"Expected navis.TreeNeuron, navis.NeuronList (len 1), or pd.DataFrame, got {type(skeleton)}")

    # Get node table
    nodes = skeleton.nodes if isinstance(skeleton, navis.TreeNeuron) else skeleton

    # Generate list of segments (edges)
    ...
    stack = np.transpose(stack, (0, 2, 1))  # shape: (n_edges, 2, 3)

    # Convert from nanometers to voxels (FlyWire: [4, 4, 40] nm)
    stack = stack / np.array([4, 4, 40])

    # Add as annotation layer (assumes add_annotation_layer exists)
