...
def test_get_mesh_neuron_single():
    ...
    n = mesh.get_mesh_neuron(TEST_ROOT_ID, dataset=None, threads=1)
...
def test_get_mesh_neuron_batch_multiple_threads():
    ...
    nlist = mesh.get_mesh_neuron(TEST_ROOT_IDS, dataset=None, threads=2)
...
def test_get_mesh_neuron_batch_single_thread():
    ...
    nlist = mesh.get_mesh_neuron(TEST_ROOT_IDS, dataset=None, threads=1)
    assert hasattr(nlist, '__iter__'), "Returned object should be iterable."
    assert len(nlist) == len(TEST_ROOT_IDS)
    for n in nlist:
        assert hasattr(n, 'trimesh')
...
def test_get_brain_mesh_scene_single_neuron():
    ...
    assert len(plotter.actors) >= 2, "Should have brain mesh and at least one neuron mesh"
...
def test_get_brain_mesh_scene_multiple_neurons():
    ...
    assert len(plotter.actors) >= len(TEST_ROOT_IDS_MIXED) + 1, "Should have brain mesh and multiple neuron meshes"
